...
public class Fetch
{
    ...
    public Fetch( CallbackSuccessHandler callback, CallbackFailureHandler failCallback, RawResponse rawResponse, long matchID )
    {
        this.failureCallback = failCallback;
        this.successCallback = callback;
        this.rawResponse = rawResponse;

        ParseFromString( MatchInfoStrings.GetInstance().GetMatchInfo( matchID ) );
    }
    ...
    /// 
    /// "Assets/Resources/BackupChampionData/champion.json"
    /// </summary>
    public void ParseFromFile( string fileLocation)
    {
        string[] json = System.IO.File.ReadAllLines(@fileLocation);
    ...
    public void ParseFromString(string jsonString)
    {
        ...
        else
        {
            if (successCallback != null)
            {
                successCallback(rawResponse(jsonString));
            }
        }
    }
